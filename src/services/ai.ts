...
export interface AiIntent {
  action: "add" | "list" | "complete" | "delete" | "move" | "chat";
...
- "move"  = อยากย้ายงานข้ามฐาน (เช่น "ย้ายงาน X ไปสถานธรรม" / "อันนี้เป็นงานส่วนตัว ย้ายไปงานธรรม") → กรอก title (งานที่จะย้าย) + database (ปลายทาง: task=ส่วนตัว, project=สถานธรรม)
...
สำคัญ: action "add" และ "move" ต้องกรอก "database" เสมอ (task=ส่วนตัว, project=สถานธรรม) ห้ามเว้นว่าง
...
- "ย้ายงานเอาผ้าไปอบไปเป็นงานส่วนตัว"
  → {"action":"move","title":"เอาผ้าไปอบ","database":"task","reply":"ย้ายมางานส่วนตัวแล้วค่ะ"}`;

const INTENT_SCHEMA = {
  type: "object",
  properties: {
    action: { type: "string", enum: ["add", "list", "complete", "delete", "move", "chat"] },
